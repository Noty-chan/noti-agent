...
- [ ] Добавить единый entrypoint локального запуска для разработчика без Docker:
...
- [ ] Подготовить кросс-платформенные скрипты:
  - `scripts/setup.ps1` (Windows приоритет),
  - `scripts/setup.sh` (Linux).
- [ ] Убрать обязательность Telegram из first-run профиля (VK + LLM + SQLite + Mem0/Qdrant как обязательные/условно-обязательные по флагам).
- [ ] Добавить минимальный Quick Start в README (без Docker).
...
- [ ] Добавить web-панель (например FastAPI + минимальный UI) в модуле `noty/config/`:
...
- [ ] Реализовать безопасное сохранение в `.env`/конфиги и reload runtime-настроек.
- [ ] Добавить кнопки: «Сохранить», «Запустить Noti», «Остановить Noti», «Статус сервиса».
- [ ] Добавить базовую авторизацию для локальной панели (пароль из `.env`).
...
- [ ] Добавить отдельный модуль заметок: `noty/memory/notebook.py` + таблицу SQLite `noti_notebook`.
- [ ] Реализовать tool-call интерфейс для Ноти:
  - `notebook_add`, `notebook_update`, `notebook_delete`, `notebook_list`.
- [ ] Ограничить размер блокнота (жёсткий лимит записей/символов) и явно сообщать лимит в prompt слое.
- [ ] Встроить notebook в Dynamic Context Builder как приоритетный «не забыть» слой.
- [ ] Логировать изменения блокнота в `.jsonl` архив.
...
- [ ] Добавить слой скользящей памяти последних N дней: `noty/memory/recent_days_memory.py`.
- [ ] Механика затухания важности: свежие события влияют сильнее, старые медленно ослабевают.
- [ ] Интегрировать слой в Dynamic Context Builder отдельно от semantic/recent/important, чтобы формировать устойчивую «личность во времени».
- [ ] Добавить периодический maintenance-job (сжатие/очистка/пересчёт весов).
- [ ] Обновить метрики и логи: доля фактов из rolling memory в финальном контексте.
